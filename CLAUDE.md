...
| `CLAIM_CODE`         | Optional: pin a specific claim code instead of letting the orchestrator mint one (WARP-632 / ADR-017). When set, `npm run seed` also hashes and seeds it at provisioning time so `POST /api/setup/claim` works out-of-box on headless/no-PyPortal devices (WARP-637 interim). Leave empty on any box where the orchestrator should mint the code at runtime. |
