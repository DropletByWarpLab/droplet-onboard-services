...
services/pm/            Python FastAPI sidecar wrapping upstream Plane (AGPL-3) — embedded PM stack per ADR-007
...
| `DROPLET_PM_API_URL` | Plane backend API URL on the compose network (default `http://pm-api:8000`) — embedded PM stack per ADR-007 |
...
| `DROPLET_PM_WEBHOOK_SECRET` | HMAC signing key for Plane → orchestrator webhooks (WARP-511). Empty default → webhook receiver fail-CLOSED per `security-rules.md` |
