...
Smart-home control runs through the `matter-controller` host-network sidecar (`services/matter-controller/`, ADR-021), fronted by the orchestrator's `/api/matter/*` routes (`apps/orchestrator/src/services/matter.service.ts` is the HTTP client).
