...
/**
 * WARP-3153: stand-in for device-identity-svc. node-forge only builds RSA
 * certs; the real device key is ECDSA P-256, and the verifier is
 * algorithm-agnostic (node:crypto `verify` with the cert's public key).
 */
const DEVICE = (() => {
  ...
  cert.validity.notAfter = new Date("2031-01-01T00:00:00Z");
  const name = [{ name: "commonName", value: "droplet-test-device" }];
...
const sealer: ActivityBundleSealer = {
  ...
  async signWithDeviceKey(payload) {
    ...
    s.update(payload);
    return { signature: s.sign(DEVICE.privatePem), algorithm: "RSA-SHA256" };
  },
};
...
describe("POST /api/activity/export", () => {
  ...
  it("returns a JSON-Lines body with manifest + rows in ascending id order", async () => {
    ...
    expect(lines[4].rowCount).toBe(3);
  });
  ...
  it("carries no private key material: no PEM private-key marker, no HMAC key, no device key", async () => {
    ...
    // The device private key body (base64 DER between the PEM markers).
    const derB64 = DEVICE.privatePem.replace(/-----[^-]+-----|\s/g, "");
    expect(text).not.toContain(derB64.slice(0, 64));
  ...
  it("the verifier rejects an edited row, a re-sealed forgery and a wrong device", async () => {
    const res = await request(signedApp()).post("/api/activity/export").send({});
    const fp = `sha256:${createHash("sha256").update(DEVICE.certPem, "utf8").digest("hex")}`;

    const edited = res.text.replace("event 2", "event X");
    expect(runVerifier(edited, "--fingerprint", fp).code).toBe(1);

    // A forger with their own key can make a self-consistent bundle, but it
    // no longer matches the box's fingerprint.
    expect(runVerifier(res.text, "--fingerprint", "sha256:" + "0".repeat(64)).code).toBe(1);
  });

  it("the seal reports rows whose HMAC the box could not verify", async () => {
    ...
    const seal = JSON.parse(res.text.trim().split("\n").at(-1)!);
    expect(seal.rowHmac).toEqual({ checked: 3, failed: 1, failedRowIds: ["2"] });
    expect(runVerifier(res.text).code).toBe(1);
  });
  ...
  it("writes an unsigned seal (which fails verification) if signing breaks mid-export", async () => {
    const broken: ActivityBundleSealer = {
      getDeviceCert: sealer.getDeviceCert,
      async signWithDeviceKey() {
        throw new Error("tpm wedged");
      },
    };
    const res = await request(makeApp(signedChain(2), broken))
      .post("/api/activity/export")
      .send({});
    const seal = JSON.parse(res.text.trim().split("\n").at(-1)!);
