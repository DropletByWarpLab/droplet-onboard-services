...
// Resolve the schema path by walking up from process.cwd() until we
// find a `prisma/schema.prisma`. Vitest can be invoked from either the
// orchestrator package (npm test) or the repo root (npm run
// test:orchestrator); both must work. We deliberately don't use
// import.meta.url because the orchestrator compiles to CommonJS, which
// forbids that property.
