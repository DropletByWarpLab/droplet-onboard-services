...
/** One visible after-hours sighting, as the recipient may know it. */
...
export function alertCopy(input: { zoneName: string; evidence: readonly AlertEvidence[]; tz: string | null }): {
  ...
  const sorted = [...input.evidence].sort((a, b) => a.at.getTime() - b.at.getTime());
  const lead = sorted[0]!;
  ...
  return {
    title: `Person in ${safe(input.zoneName, "an area")} after hours`,
    body: `${camera} saw someone${when}. The site was ${MODE_WORDS[lead.mode] ?? "closed"}.${tail}`,
  };
}
