...
function appAs(user: Record<string, unknown> | undefined) {
  const app = express();
  ...
  app.use("/api", createToolsRuntimeRouter({ policy: policy(), list: () => TOOLS }));
  return app;
}
...
describe("GET /api/llm/tools/runtime", () => {
  ...
  it("an empty runtime layer is an empty list, not an error", async () => {
    ...
    app.use("/api", createToolsRuntimeRouter({ policy: policy(), list: () => [] }));
