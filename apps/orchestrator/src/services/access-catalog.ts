 ...
 *
 * WARP-2761 resolved the collision that produced — five role templates
 * granting `crm`, three of them family/guest-based — by dropping those
 * grants, NOT by returning empty domains from here. This is a live term in
 * the effective-access intersection (effective-access.service.ts
 * `reachable ∩ featureDomains ∩ granted`), so widening it would leave family
 * and guest holding a standing grant on every toolless domain, and the FIRST
 * tool a remote catalog registers into one may be a write — the exact case
 * this filter exists to catch. Deciding otherwise is a change to the write
 * filter and belongs in its own ticket; `access-catalog.test.ts` pins the
 * current answer so it cannot be reversed by accident.
 */
