...
export function domainsForFeatures(
  ...
  /** WARP-2897 — runtime-only domains (tool-layers.service.ts). No module
   *  claims them, so like every unclaimed domain they pass unconditionally;
   *  their reach is governed by the grant and write-filter terms. */
  runtimeDomains: Iterable<string> = [],
): Set<string> {
  const out = new Set<string>(UNCLAIMED_DOMAINS);
  for (const domain of runtimeDomains) {
    if (!MODULE_BY_DOMAIN.has(domain)) out.add(domain);
  }
