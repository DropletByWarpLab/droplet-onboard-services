...
/**
 * WARP-488: lowercase-hex UUID v4-shaped regex. We use UUIDs at v4 default
...
/**
 * WARP-488 — one-shot boot migrator that flips pre-WARP-485 username-keyed
 * brain-memory directories to UUID-keyed names.
 *
...
export async function migrateBrainMemoryDirectoryLayout(
  ...
  for (const entry of entries) {
    ...
    if (!userRow) {
      ...
      logger.warn(
        ...
        "WARP-488: brain-memory dir has no matching User.nextcloudUsername — operator review needed (not deleted)",
      );
    ...
    if (uuidDirExists) {
      logger.warn(
        ...
        "WARP-488: cannot rename brain-memory dir — UUID-named target already exists (operator merge required)",
      );
    ...
    logger.info(
      { brainRoot, username: name, uuid: userRow.id },
      "WARP-488: renamed brain-memory dir username → UUID",
    );
    result.renamed += 1;
  }
  ...
  if (
    ...
    logger.info(
      ...
      "WARP-488: brain-memory directory layout migration complete",
    );
  }
