...
// ── WARP-234: rediss:// connections pin trust to the internal CA ──────────
describe("redisConnectionOptions (WARP-234)", () => {
  it("returns no TLS options for a plaintext redis:// URL", async () => {
    const { redisConnectionOptions } = await import("./cache.service.js");
    expect(redisConnectionOptions("redis://:pw@cache:6379", () => Buffer.from("x"))).toEqual({});
  });

  it("pins the internal CA for a rediss:// URL", async () => {
    const { redisConnectionOptions } = await import("./cache.service.js");
    const ca = Buffer.from("---CA---");
    const opts = redisConnectionOptions("rediss://orchestrator:pw@cache:6380", () => ca);
    expect(opts.tls?.ca).toBe(ca);
  });

  it("reads the CA from DROPLET_TLS_CA / the WARP-236 bundle default", async () => {
    const { redisCaPath } = await import("./cache.service.js");
    ...
    if (prev === undefined) delete process.env.DROPLET_TLS_CA; else process.env.DROPLET_TLS_CA = prev;
  });
});
