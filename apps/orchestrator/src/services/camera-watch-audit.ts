 ...
 *
 * Opening a live view, playing an event clip or playing a recording writes
 * one signed activity row (kind `camera`): the actor, the camera, what they
 * watched and when. Reading the log is owner/admin only (routes/activity.ts).
 *
...
const WHAT: Record<CameraWatchKind, string> = {
  live: "Watched a camera live",
  clip: "Played an event clip",
  recording: "Played a camera recording",
  snapshot: "Viewed a camera snapshot",
};
