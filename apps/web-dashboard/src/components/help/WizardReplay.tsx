 ...
 * looking to LearnMoreCards used in the wizard — same indigo `.card`
 * chrome, same colour tokens, same plain-language tone (ADR-002
 * offline-first copy rule, no cloud language).
 */
    ...
    >
      <div>
        ...
        <div className="p-5 space-y-3">
          <Card
            icon={<FolderOpen size={18} className="text-[var(--brand)]" />}
          ...
          />
          <Card
            icon={<Cpu size={18} className="text-[var(--brand)]" />}
          ...
          />
          <Card
            icon={<Video size={18} className="text-[var(--brand)]" />}
  ...
  return (
    <div className="card !p-4 flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        <p className="type-subheadline mb-1" style={{ color: "var(--text)" }}>{title}</p>
        <p className="type-footnote" style={{ color: "var(--text-muted)" }}>{body}</p>
      </div>
    </div>
  );
}
