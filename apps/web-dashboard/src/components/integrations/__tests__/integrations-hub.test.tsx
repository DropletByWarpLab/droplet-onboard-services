...
describe("entries are the union of the catalog and the response", () => {
  ...
  it("catalog-only providers still render when nothing is connected", async () => {
    ...
    expect(renderedNames(container)).toEqual([
      ...
      "Square",
      "Cal.com",
      // WARP-2917 — GitLab, the third REST vendor, at catalog.order 14.
      "GitLab",
  ...
  it("every row in the response reaches some tile", async () => {
    ...
    // Fifteen catalog tiles (four original, the five WARP-2214 vendors — Xero
    // included — the three wave-1 vendors, the two WARP-2707 REST vendors, and
    // GitLab from WARP-2917) absorb four of the rows; the two the catalog knows
    // nothing about each get their own. WARP-2659 adds the MCP-track tile,
    // which this fixture reports no row for — it renders from the registry
    // regardless, which is the point.
    expect(tiles(container)).toHaveLength(18);
    for (const name of [
      "Eaglesoft",
      "Dentrix",
      "QuickBooks",
