...
describe("entries are the union of the catalog and the response", () => {
  ...
  it("every row in the response reaches some tile", async () => {
    ...
    // Sixteen catalog tiles (four original, the five WARP-2214 vendors — Xero
    // included — the three wave-1 vendors, the two WARP-2707 REST vendors,
    // the WARP-2916 GitHub one and the WARP-2917 GitLab one) absorb four of
    // the rows; the two the catalog knows nothing about each get their own.
    // WARP-2659 adds the MCP-track tile, which this fixture reports no row
    // for — it renders from the registry regardless, which is the point.
    expect(tiles(container)).toHaveLength(19);
    for (const name of [
      "Eaglesoft",
      "Dentrix",
      "QuickBooks",
