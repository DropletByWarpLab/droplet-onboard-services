...
**Honest trust boundary for `droplet`'s group membership (WARP-2888).** The paragraph above says privilege is gated by the operator's sudo password. That is only half the picture, and stating it without the rest is misleading: `droplet` is a member of the **`docker`** group, which is root-equivalent (`docker run --privileged --pid=host … nsenter -t 1 …` yields a host-root shell with no password). So on a shipped box, *any* code that can act as `droplet` — an interactive SSH session, a droplet-owned cron/script, or a compromised container that escapes to the host as `droplet` — can become root **without** the sudo password. **"`droplet` cannot sudo" is therefore not a containment boundary.** The real boundaries are (1) *who can become `droplet`* — SSH-key custody and the device-bridge's auth-gate + `NoNewPrivileges` sandbox — and (2) the narrow polkit/`EnvironmentFile` disciplines that keep the *sandboxed* bridge from reaching root (WARP-843, `50-droplet-device-bridge.rules`).

The `docker` membership is **load-bearing and kept for now**: `droplet.service` boots the whole stack as `User=droplet, Group=docker` (`docker compose up -d`), and the device-bridge's ADR-023 host wrappers (`droplet-tls-reload.sh`, `droplet-factory-reset.sh` → `scripts/lib/tls-reload.sh`) run `docker compose` as `droplet` with **no** sudo — because there is deliberately no docker sudoers grant. Removing `droplet` from `docker` without first routing all of those through root would darken the box at next boot and break TLS renewal, factory reset, and the Wi-Fi QR read. Closing this gap is a staged posture change (run `droplet.service` bring-up as root; move the bridge's docker calls to root executors like the existing mdadm/netifd polkit units; then strip the account-level `docker` membership) tracked on WARP-2888 — not a change to make piecemeal on a boot-critical path. What this ADR *does* fix now is the gratuitous half: `droplet` is also placed in the base Ubuntu image's **`lxd`** group, which is equally root-equivalent and entirely **unused** by Droplet; the `droplet-firstboot` `ExecStartPost` now removes it (`gpasswd -d droplet lxd`) alongside the sudoers teardown. `tests/droplet-user-privileged-groups-invariant.test.sh` guards both facts: that no Droplet code ever adds `droplet` to `lxd` (or any root-equivalent group other than `docker`), and that the only shipped unit granted the `docker` group is `droplet.service`.
