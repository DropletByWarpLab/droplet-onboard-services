...
- **Supersedes / relates:** ADR-002 (home/small-team persona), ADR-004 (RBAC per-route guards), ADR-011 (hardware-agnostic), ADR-013 (built-in directory is identity source of truth; Nextcloud demoted to downstream WebDAV), ADR-021 (container resource limits), ADR-023 (per-device FQDN/TLS)
...
- `apps/orchestrator/src/routes/llm.ts:748-759` — FileCitation writes `userId = req.user.id` (UUID, explicit)
...
- `apps/orchestrator/prisma/migrations/20260428000000_brain_memory/migration.sql:24-32` — idempotent enum idiom (`DO $$ BEGIN CREATE TYPE … EXCEPTION WHEN duplicate_object THEN null; END $$;`)
