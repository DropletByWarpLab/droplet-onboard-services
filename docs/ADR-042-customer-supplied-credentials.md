...
| **Loyverse** | A **personal access token** from the owner's own Back Office (Integrations → Access tokens), on the free account — "Integrations" is a free feature | An opaque token in `Authorization: Bearer`. **No documented prefix or length**, so the box refuses an empty value and nothing else. Up to 20 tokens per account | No — but the **OAuth 2.0 app** (client id + secret, 43 200 s token refresh) is the other path, and it is ADR-042 model 2, so the box does not offer it | **None. "The token gives unlimited access to all resources provided by the Loyverse API" — read AND write.** No read-only kind exists on this path (scopes exist only on OAuth). The box narrows itself to two GET endpoints (customers, items); the token does not. | **Optional — the owner picks an expiry at creation, or none.** | 2026-09-18, WARP-2919 |
...
| Loyverse | any non-empty token | **nothing by shape** — Loyverse documents no prefix and no length for a personal access token, so a regex here would be a guess that refuses valid tokens |
...
| Loyverse | The account owner, in their own Loyverse Back Office | **No** — the OAuth developer-dashboard app, which WOULD be a Warp-Lab registration, is deliberately not used |
...
| Loyverse | `api.loyverse.com` | `kind: egress`, `data_class: user-content-on-request` — one static host; the `/v1.0` version is a path, never part of the host (WARP-2919) |
