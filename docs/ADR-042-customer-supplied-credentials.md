...
| **Atlassian** | An **API token** created at id.atlassian.com by the account whose access the box should have, plus that account's email and the site's `cloudId` | email + API token, presented as `Authorization: Basic base64(email:token)`; `cloudId` carried as an argument on every call | No — the API token is the only shape on the headless path | **None. The token is unscoped**: it carries the full permissions of the account that created it, across every product and every site that account can reach. Access is limited by *whose* account creates it, and by the box's own read-only classification table. | **Yes — 365 days maximum**, no grace period and no refresh. The only row on this table with a hard stop. | 2026-09-02, WARP-2316 |
...
| **GitLab** (gitlab.com) | A **personal access token** from the owner's own profile (Edit profile → Access → Personal access tokens), on any tier including Free | A `glpat-…` token in GitLab's own `PRIVATE-TOKEN` header — never the `private_token` query form, never `Bearer`. 🔴 **Do not pattern-match it:** gitlab.com tokens are now *routable* (`glpat-<27..300>.<2>.<9>`, two dots) and the obvious `^glpat-[A-Za-z0-9_-]+$` rejects every one of them; the prefix is admin-configurable on self-managed besides | No — but the **`api`** scope (full read-write) is the confusable one; the guide asks for **`read_api`** (Legacy token) or **Work Item: Read + User: Read** (Fine-grained token) | Per-scope on a Legacy token; per-permission on a Fine-grained token. The owner *can* narrow it, and the guide asks them to | **Yes — required, default and default-maximum 365 days.** The second row with a hard stop, after Atlassian. | 2026-09-18, WARP-2917 |
...
- **Brevo breaks the "these do not expire" assumption that `credential-handling.md` states for every other vendor here.** Two independent clocks: a lifetime the owner chooses at creation, and a **90-day inactivity retirement**. The second is the dangerous one — a connection that works today can stop on its own with nothing changed and no action by the customer, which presents as a broken integration rather than an expired key. Any "credentials on this box do not expire" copy must exclude Brevo.
- **Pipedrive's real gate is a permission switch, not a plan.** API access is controlled per permission set, and when it is off the API page is simply **absent from the menu** — no error, no greyed-out control. It is indistinguishable from having looked in the wrong place, Droplet cannot detect it in advance, and only an administrator can turn it on. It is the most likely reason a Pipedrive connection never gets started.
...
- **Atlassian is the only credential on this table that EXPIRES, and the only one with no scope model at all.** Both facts change what the box has to build rather than just what the guide has to say. The 365-day stop means expiry is an explicit status — never `VALID` inside 30 days, and "stored but no expiry recorded" is its own value (`EXPIRY_UNKNOWN`) rather than an optimistic default. It is derived by `credentialExpiryVerdict` in `packages/shared-types/src/provider-descriptor.ts` from the track's declared `credentialExpiry` policy, so it carries no vendor name; `saas-credential.service.ts` gates it on a credential actually being stored, because "nothing is connected" is a different state from "connected, no date recorded" and only the second is a warning. (WARP-2353 originally shipped this as a second, orchestrator-only module; it had no production callers and was deleted in WARP-2300 rather than left to be kept in step with this one by an assertion.) The absent scope model means the credential cannot be narrowed at intake the way §4 narrows Stripe's, so the narrowing happens on our side instead: an operator-owned classification table decides which of the server's tools may run, and in v1 that is reads only (`atlassian-tool-policy.ts`, snapshotted at `docs/security/atlassian-mcp-tool-surface.json`). Atlassian is also the first entry here that is **not an ADR-041 cloud connector at all** — it is an outbound MCP session under [`ADR-043`](ADR-043-outbound-mcp-client.md), and it appears on this table because the credential model is identical even though the transport is not.
...
| GitLab | any non-empty token | **nothing by shape** — the token format has already changed once (20-character → routable dotted) inside one product generation, and the spec's own `^glpat-[A-Za-z0-9_-]+$` would have refused every token gitlab.com mints today; the `glpat-` prefix is named in the help text, not enforced |
...
| GitLab | The account owner, in their own gitlab.com profile | **No** — project access tokens exist but are Premium/Ultimate-only on gitlab.com, so the personal token is the free-tier path |
...
| GitLab | `gitlab.com` | `kind: egress`, `data_class: user-content-on-request` — one static host (the API is under `/api/v4` on the product's own host); self-managed GitLab would be `kind: dynamic` on its own ticket |
