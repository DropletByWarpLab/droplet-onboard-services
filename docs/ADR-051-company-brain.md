...
**The consent language is the deliverable, not the button.** It is stated at the moment of the click and is deliberately unflattering: the pass reads *with whichever model the box is set to use, and if that is a cloud model the text is sent to that provider*; it is *slow on purpose* (~240 documents/day against one inference slot); and turning it off *stops the passes without deleting what has already been written* — to which WARP-2876 adds *"and the assistant stops bringing it up in chat"*, because until that gate landed the second half of that promise was the only half the code kept. The first of those three is the one that must never be softened into "it stays on the box" — the corpus pass resolves `DEFAULT_MODEL ?? LLM_MODEL` through the AI gateway, which routes cloud model names to cloud providers. *(WARP-3047: the pass now resolves the box's active model — the Models page choice, LLM_MODEL as fallback — which is always a local model.)*
...
**But the PROMPT-INJECTED path is gated, and the two are not the same surface (WARP-2876).** As WARP-2838 shipped it, the switch gated the **write** path only: `index.ts`'s `preconditions` stop both passes, so a revoked brain stops *reading* the business's documents. Nothing gated the **tell** path. `buildBrainBlock` never consulted the switch, and it is called on every turn by `routes/llm.ts` and by `prompt-inspect.service.ts`, so after an owner revoked consent every chat turn was still handed a system-prompt block assembled from rows derived from their documents before the revocation. The click stopped the reading and not the telling, and the telling is the half that reaches the model.

The line between the two, stated so the gate is not "simplified" back out: **`/brief` and `business_find` are asked for by a person; the brain block is injected into every turn whether or not anyone wanted it.** Revoking consent withdraws the unbidden path and leaves the owner's own data visible to the owner. So `GET /api/brain/findings` stays role-gated and never brain-gated, `/brief` keeps rendering the rows, and `buildBrainBlock` returns `""` while the switch is off — fetching nothing, since a block assembled and then dropped has still pulled revoked-consent content into the process.

The gate lives **in `buildBrainBlock`**, not at its two call sites: both callers funnel through it, a third caller is the likely shape of the next regression, and it is where `visibleScopeFilter` already lives for the same reason — consent and scope are both "who may be told what". It is asked **per call** (the switch is a row an owner flips mid-session) and **fails closed** (an unreadable switch sends no block, matching the unresolvable-scope path beside it).

**It is a serve gate and not a purge, and that is this section's ruling, not an omission.** Deleting `BrainDigest` / `BrainFinding` on the off click was considered and rejected: it contradicts the paragraph below, and it would make the box delete the owner's data while the sentence beside the button said it would not. Retention and deletion remain the open question recorded in §11 — a *deliberate* action with its own affordance, never a side effect of a consent toggle.
...
- **No dedicated brain model.** The box's active model (WARP-3047; was `DEFAULT_MODEL` then `LLM_MODEL`), through `ai-gateway`; with neither set the corpus pass is **skipped**, rather than scheduled to fail hourly and fill `lastError` with noise.
...
- **Retention and deletion for `BrainDigest` / `BrainFinding`.** `MoneySnapshot` has a retention policy; the brain tables do not. Nothing currently deletes a finding, ever. WARP-2876 sharpened rather than answered this: the consent switch is now known *not* to be the deletion affordance (§9.9), so the owner-facing "until you delete it" still has no door behind it.
