...
`BRAIN_ENABLED` **defaults off** and gates pass seeding and both ticks. It is parsed by an explicit string→bool transform accepting only `"1"` / `"true"`: `z.coerce.boolean()` runs `Boolean(...)`, so an operator writing `BRAIN_ENABLED=false` to opt **out** would have switched it **on**.
