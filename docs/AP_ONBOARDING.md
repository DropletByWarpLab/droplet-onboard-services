...
6. **You can change its Wi-Fi afterwards, from the dashboard.** An approved AP's network name and password are editable from **Network → Wi-Fi** and from the AP's own card under **Coverage Extenders** — the same control in both places, so they can never show different values. The card also shows the AP's model, firmware, uptime and per-radio channel/width/connected devices, read live off the AP. The password shown is the per-unit one the AP minted at first boot (`/etc/droplet/wifi-psk`) — you never need to SSH in to read it. Because the AP is infrastructure, it deliberately does **not** also appear in the **Devices** list; it lives under Coverage Extenders only.
