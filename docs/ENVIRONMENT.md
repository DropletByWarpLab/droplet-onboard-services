...
| `COMPOSE_PROFILES`   | Which optional service groups start (`linux`, `display`, `full`, `single-box`, `eval`, `ops`). Written by `setup.sh` — semantics per shape in the `docker-stack` skill |
