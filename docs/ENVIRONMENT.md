...
| `LLM_NUM_CTX` | Effective context window (tokens) the **orchestrator's** request-size estimator budgets against before sending a chat request (WARP-1118 §10, `context-budget.service.ts`). When the assembled request (system blocks + serialized `tools[]` + pins + attachments + history) would exceed `LLM_NUM_CTX − 1024` (output reserve), the orchestrator deterministically drops the business block, then the persona block, then falls to history/attachment trimming — preventing, not just detecting, the WARP-854 overflow. Default `4096` to match Ollama's own default. **On the single-box shape** the server is configured wider via `OLLAMA_CONTEXT_LENGTH` (16384); set `LLM_NUM_CTX` to match the deployed server window so the estimator doesn't degrade blocks the model could actually carry. The on-hardware measurement + final value (§10a) is a separately gated step. |
