...
| `AGENT_MAX_ITER_DEFAULT` / `AGENT_MAX_ITER_CAP` | Agent-loop step limit (agent-budgets spec §1) — the default iteration count a turn uses when the caller doesn't set `max_iter`, and the hard cap no caller (including an explicit `max_iter`) can exceed. Both enforcement points (the `llm-agent.service.ts` clamp and the `routes/llm.ts` zod bound) read the same config values so they can't drift. Defaults `5` / `10` (today's hard-coded behavior). If `AGENT_MAX_ITER_CAP` is configured below `AGENT_MAX_ITER_DEFAULT`, boot clamps the default down to the cap and emits a structured warning. Leave both at their defaults until the spec §6 phase-2 step-sweep measurement (5 → 8 → 10) picks new values. |
