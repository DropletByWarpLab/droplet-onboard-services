...
|---|---|---|---|
| [WARP-559](https://warp-lab.atlassian.net/browse/WARP-559) | Highest | Managed-switch control API has **no authorization guard** — any authenticated session (incl. `guest`) can disable ports/PoE/VLANs | `app.ts:30` mount unguarded; `switch.ts` 0× `requireScope`/`requireRole` across 10 mutating routes |
...
| [WARP-563](https://warp-lab.atlassian.net/browse/WARP-563) | High | MCP server grants **full tool trust from absence of claims** (state-by-absence anti-pattern) | `mcp-server/src/server.ts:30` |
