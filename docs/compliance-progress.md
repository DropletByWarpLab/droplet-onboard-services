...
| FIPS 140-3 cryptographic modules | 🟡 in progress | WARP-229 closed: provider activation apparatus + boot self-tests + PR-blocking lint shipped. WARP-967: validated `fips.so` (OpenSSL 3.0.9, CMVP #4282) source-built into every shipped service image with a build-time KAT self-test; no Alpine base remains. Pending: runtime activation via setup.sh (WARP-318). |
...
| Vulnerability + supply chain | 8 | 0 | 1 | 7 |
