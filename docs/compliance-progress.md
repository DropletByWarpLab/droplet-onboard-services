...
- **2026-07-09** — **WARP-1063 ✅ FIPS activation now boots a FIPS-enforcing stack.** Root-caused the WARP-317-surfaced boot block: it was never a missing `ssl_conf` cipher list — the validated provider was failing to ACTIVATE in the second libcrypto instance of multi-OpenSSL processes (Node bundled OpenSSL + Prisma's system libssl in orchestrator/mcp-server; pyca `cryptography`'s static OpenSSL + system libssl in ai-gateway/file-indexer). The FIPS module cannot be initialized by two libcryptos resolving the same `fips.so` (upstream openssl#25553, `common libcrypto routines::init fail`); the process-wide `OPENSSL_MODULES` forced that collision, and under the `fips=yes` property pin the losing instance had ZERO fetchable algorithms → `LIBRARY_HAS_NO_CIPHERS`/P1011 at TLS-client construction while every property-based self-test signal stayed green. Fix: (1) per-runtime `fips.so` COPIES installed at each bundled runtime's baked MODULESDIR (`install-fips-provider.sh` + Dockerfiles, with build probes that fail on drift); (2) `OPENSSL_MODULES` removed from the derived env (`apply_fips_mode` deletes it; an empty value is itself harmful); (3) explicit FIPS-approved TLS posture pinned on the default context in `docker/openssl-fips.cnf` (`ssl_conf`/`system_default`, AES-GCM only, lockstep with the WARP-1021 edge profile); (4) both boot self-tests gained a POSITIVE probe (approved digest must WORK) so a dead provider fails the boot with the real diagnosis; (5) `tests/integration/fips-stack.test.sh` now asserts the full FIPS-enforcing boot + API endpoints unconditionally (the documented-exception branch is gone). On-hardware validation remains deferred to the appliance bring-up.
- **2026-07-05** — **WARP-317 ✅ done + surfaced an open FIPS-config defect.** `docs/fips.md` (activation, verification commands, dev opt-out, the "library has no ciphers" failure mode + diagnosis, enforcement-scope table) linked from README/CLAUDE.md/ENVIRONMENT.md; `tests/integration/fips-stack.test.sh` boots the compose stack with `DROPLET_FIPS_MODE=1` (via the real `setup.sh --fips`) and asserts the parts of activation that genuinely work — per-service `fips_self_test fips:true`, FIPS-restricted edge TLS (AES-GCM negotiated, ChaCha refused), in-container MD5 refusal in both stacks, and non-provider images staying off the boot gate — wired as the path-gated `fips-stack` job in `test-fips.yml`; Docker-free harness checks in `tests/fips-stack-logic.test.sh`. Carries the WARP-318 collateral fix: compose pins `DROPLET_FIPS_REQUIRED=false` (+ stock `OPENSSL_CONF` where env_file leaks) for the five images without the validated module; email-indexer's legacy `:-true` default (rendered `true` even with the knob OFF on a key-less .env) removed. **Finding — booting under FIPS is currently blocked** for services that construct a TLS client at startup (orchestrator Prisma `$connect` → P1011; ai-gateway httpx → LIBRARY_HAS_NO_CIPHERS): the shared `docker/openssl-fips.cnf` activates only fips+base with `default_properties=fips=yes`, leaving a default client SSL_CTX with no ciphersuites. The provider itself loads + enforces; the fix is a provider-config change (add an `ssl_conf`/`system_default` section — WARP-967/WARP-318 territory), after which `EXPECT_FIPS_STACK_BOOTS=1` flips the smoke test to assert the clean boot + API endpoints. The WARP-233 Postgres-TLS clash stacks on top of that. (This corrects the merged WARP-318 comment that claimed the orchestrator reaches Postgres via a plaintext fallback — CI proved it does not.)
