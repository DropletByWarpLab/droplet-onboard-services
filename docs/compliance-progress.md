...
- **2026-07-05** — **WARP-317 ✅ done.** FIPS activation made provable end-to-end: `docs/fips.md` (activation, verification commands, dev opt-out, the "library has no ciphers"/P1011 failure mode + diagnosis, enforcement-scope table) linked from README/CLAUDE.md/ENVIRONMENT.md; `tests/integration/fips-stack.test.sh` boots the full compose stack with `DROPLET_FIPS_MODE=1` and asserts per-service `fips_self_test` lines, FIPS-restricted edge TLS (AES-GCM negotiated, ChaCha refused), live API endpoints, in-container MD5 refusal in both stacks, and zero crash-loops — wired as the `fips-stack` job in `test-fips.yml`, path-gated so ordinary PRs don't pay the boot; Docker-free harness checks in `tests/fips-stack-logic.test.sh`. Carries the WARP-318 collateral fix: compose pins `DROPLET_FIPS_REQUIRED=false` (+ stock `OPENSSL_CONF` where env_file leaks) for the five images that don't ship the validated module — email-indexer's legacy `:-true` default (which rendered `true` even with the knob OFF on a key-less .env) removed. Corrected the merged WARP-318 comments: the orchestrator DOES boot FIPS-enforcing under `--fips` and reaches Postgres because the intra-compose hop is plaintext today; the P1011 clash is deferred to WARP-233, not avoided by an orchestrator opt-out.
