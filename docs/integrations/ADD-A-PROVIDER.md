...
> **Applying a connector draft from a box (WARP-2899).**
> A Droplet box can DRAFT a declarative REST profile in the Workshop (the `rest-profile` template, ADR-056 slice L): the profile, its six-section guide, its egress entry and its ADR-042 rows, every unverified fact marked `TODO(verify)`. An owner exports it as a git bundle (`GET /api/workspace/:id/export`). The draft is **never a runtime object** — nothing on the box loads it, and it becomes a connector only through the ordinary PR this guide describes. To start that PR from a bundle: `git fetch <file>.bundle work:draft/<provider>`, then check out the rendered files from `draft/<provider>` and follow the draft's own `DRAFT-CHECKLIST.md` (registration in `profiles.ts`, the descriptor, the guide pins, §7b's four lists, the vendor test, and the egress entry under security review).
