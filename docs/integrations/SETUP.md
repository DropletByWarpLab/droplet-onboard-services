...
> **Not every credential in this table is scoped, and the unscoped ones are the majority.** Stripe, Klaviyo, Shopify, GitHub and GitLab let you narrow what the credential may do — GitHub most finely of all, repository by repository. Atlassian, Mailchimp, Brevo, Pipedrive, Square, Cal.com and Todoist do not — each carries the full access of whoever created it, so on those *whose* account creates it is the access decision, and there is nothing to tick. Droplet limits itself instead, to reads only, from a list held in the product; each guide says plainly which of the two you are getting.
