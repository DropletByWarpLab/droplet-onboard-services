...
* **Nothing is read outside the drop root.** The root is resolved once; every candidate is resolved (through symlinks) and re-checked against it. A symlink planted in a share the practice controls cannot become an arbitrary-file read inside the orchestrator.
