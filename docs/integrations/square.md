...
- **A payout's status can update late.** Square lets Droplet ask "what changed since last time" for payments and refunds, but the only filter on payouts is *when the payout was created* — so a payout that moves from Sent to Paid days later would never come up again. Droplet therefore re-reads payouts on a periodic full sweep instead of relying on "what's new". The status you see is right; it can become right a little after the fact.
