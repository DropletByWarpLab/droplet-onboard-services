...
| 3 | All FKs local UUIDs (grep-gate) | ✅ | `apps/orchestrator/src/routes/auth.jwt-uuid.test.ts` + `__tests__/auth.nc-password-reset-escalation.test.ts` (WARP-485/WARP-881; the OCS-path file went with the fallback in WARP-2994 — `req.user.id` is always the local `User.id` UUID at the auth layer, every route's FK writes flow from this) + `addMemberSchema.userId: z.string().uuid()` in `routes/departments.ts`, pinned by `department-security-static-gates.suite.test.ts` (f2) (WARP-1274/T21, new) |
