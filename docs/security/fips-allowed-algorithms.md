...
- Loaded via `OPENSSL_CONF=/etc/ssl/openssl-fips.cnf` (config file shared across services in `docker/openssl-fips.cnf`); Node services additionally need `OPENSSL_MODULES` pointing at the system `ossl-modules` dir (node statically links its own OpenSSL, whose baked-in module path is not Debian's) **and** `NODE_OPTIONS=--openssl-shared-config` (node reads the config under its own `nodejs_conf` appname; the flag makes it honor the shared `openssl_conf` key). Runtime activation is opt-in, per-customer, default OFF — owned by WARP-318.
