...
>
> **Status update 2026-06-10 (OQ2 amendment — dedicated origin, not subpath):** Hit reality on the single-box: the `/projects` iframe rendered **black**. The vanilla `plane-frontend` image is built with Next.js `basePath: ""` baked into the build (`routes-manifest.json`), and there is **no runtime web base-path override** — so under the prefix-stripping `/pm/` proxy, every asset URL (`/_next/*` → 404 off the **dashboard's** Next server), every browser API call (`/api/*` → the **orchestrator**, with genuinely colliding paths like `/api/users`), and every client-side route push (`/<workspace-slug>/...` → the dashboard) escapes the prefix. Subpath hosting of vanilla Plane is not implementable — not an asset-routing bug. Resolution, keeping the ADR-010 vanilla-image posture: Plane gets a **dedicated TLS origin on `:8443`** (same host, same cert; gateway server block mirrors Plane's official self-host proxy: `/` → pm-web, `/api/` + `/auth/` → pm-api). The OQ2 iframe survives: the frame is now cross-origin (port), and Plane's `X-Frame-Options: SAMEORIGIN` is dropped at **our gateway** in favor of `Content-Security-Policy: frame-ancestors 'self' https://$host` — header surgery at the proxy, **no Plane source patch**, `services/pm/PATCHES.md` stays empty. `DROPLET_PM_WEB_URL` default moves `https://droplet-ai.local/pm` → `https://droplet-ai.local:8443` (setup.sh migrates the old default in place; operator overrides preserved). OIDC callback URLs under WARP-505 become `https://<gateway>:8443/auth/oidc/...`.
