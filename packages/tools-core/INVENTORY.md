...
| analyze_file_cleanup | files | Read-only cleanup report for a folder: bounded tree walk over `GET /api/files?path=` (depth ≤ 8, ≤ 5000 entries, ≤ 300 listings); totals by category, largest, stale (`stale_days`), junk (OS/editor leftovers), duplicate candidates (normalized name + size — NOT content, that is WARP-2096), empty subfolders, and the exact `organize_files` plan for a rule. Sampled lists, exact counts. WARP-2664. | false | false | nextcloud |
| organize_files | files | Sort the files DIRECTLY inside one folder into subfolders by rule (`by_type` / `by_extension` / `by_month` / `by_year`): one listing, `POST /mkdir` per destination, `POST /move` per file with `overwrite: false` (a clash is skipped, never overwritten). Subfolders and hidden files untouched, root refused, ≤ 500 moves per call. Interceptor-owned confirmation, no `confirmed` flag in the schema. WARP-2664. | true | true | nextcloud |
| delete_files | files | Bulk delete-to-trash of an explicit path list (1–100): every path is resolved in its parent's listing first, so a folder is refused unless `allow_folders` AND empty (a folder with contents always points at `delete_file`); root refused; validation completes before the first `DELETE /api/files`. Per-path deleted/skipped/failed. Emptying the trash stays dashboard-only. Interceptor-owned confirmation. WARP-2664. | true | true | nextcloud |
