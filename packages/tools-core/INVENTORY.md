...
| business_create | business | Create a customer, deal, project, task, or a note on a customer/deal/task. Replaces pm_create_project, pm_create_work_item, pm_add_work_item_comment and crm_log_activity, and adds the customer + deal creation the CRM never had. `contact` is deliberately absent: no contact-creating route admits the mcp principal. ADR-045 slice D. | true | true | orchestrator |
| business_update | business | Change a customer, deal or task: stage/state, assignee, name. One atomic PATCH per entity — since WARP-2579 the deal PATCH applies the stage move and writes STAGE_CHANGE in the same transaction. Replaces crm_move_deal_stage, pm_update_work_item and pm_transition_work_item. `project` absent: PATCH /pm/projects/:id is mcp-blind. ADR-045 slice D. | true | true | orchestrator |
| business_link | business | Join two existing records. Two edges are live on this box (deal→project `delivers`, deal→customer `belongs_to`, both columns on CrmDeal per WARP-2117); the rest of the intended graph ships as `not_built` rows in LINK_EDGES and refuses with what it waits for. Out of default chat scope for budget. ADR-045 slice D. | true | true | orchestrator |
...
| business_find | business | One read over the business graph: customer / contact / deal / project / work_item / pipeline. With `id`, the record plus its edges; without, a search with a total. Replaces the five `crm_*` and five `pm_*` reads. ADR-045. | false | false | orchestrator |
| business_timeline | business | The merged activity feed for one customer, contact, deal or work item — CrmActivity for CRM nodes, PmActivity + PmComment for PM ones, ordered newest first. ADR-045. | false | false | orchestrator |
