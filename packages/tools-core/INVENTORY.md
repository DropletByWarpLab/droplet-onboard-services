...
| analyze_file_cleanup | files | Read-only cleanup report for a folder: bounded tree walk over `GET /api/files?path=` (depth ≤ 8, ≤ 5000 entries, ≤ 300 listings); totals by category, largest, stale (`stale_days`), junk (OS/editor leftovers), duplicate candidates naming the copy to KEEP (normalized name + size — NOT content, that is WARP-2096), empty subfolders, and per-destination move counts for a rule. Every list sampled with `shown`/count so a sample cannot read as the whole set; a test pins the serialized result under the orchestrator's 8000-char cap. Carries an explicit caveat when an empty listing (which the route's outage-degrade also produces) could explain the result. WARP-2664. | false | false | nextcloud |
| organize_files | files | Sort the files DIRECTLY inside one folder into subfolders by rule (`by_type` / `by_extension` / `by_month` / `by_year`): one fresh listing, `POST /mkdir` per destination, `POST /move` per file with `overwrite: false` (a clash is skipped, never overwritten). Subfolders and hidden files untouched, root refused, ≤ 500 moves per call. Destinations are re-validated through `validateNcPath` before use (P11) since they are built from listing-supplied names; `created_folders` reports only folders this call actually made (mkdir 200s for an existing one). Interceptor-owned confirmation, no `confirmed` flag in the schema. WARP-2664. | true | true | nextcloud |
| delete_files | files | Bulk delete-to-trash of an explicit FILE list (1–100): every path is resolved in its parent's listing first, so the tool only deletes what it has positively seen; root refused; validation completes before the first `DELETE /api/files`, so a bad entry late in the list cannot leave the earlier ones deleted. **Directories are refused outright** — an `allow_folders`-plus-emptiness guard is unsound here because `GET /api/files` answers 200 `[]` on an upstream outage, so an outage would satisfy the check and a full tree would be trashed recursively; the refusal deliberately does not name `delete_file` (which is recursive AND `requiresConfirmation: false`). Per-path deleted/skipped/failed. Emptying the trash stays dashboard-only. Interceptor-owned confirmation. WARP-2664. | true | true | nextcloud |
