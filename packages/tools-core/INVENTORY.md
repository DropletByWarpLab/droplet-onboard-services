...
| get_gpu_status | system | Live GPU counters (utilisation, VRAM used/total, power, temperature) plus the processes holding the card and the container each belongs to, via `/api/hardware/gpu` → host device-bridge `/gpu`. Route is `requireRoleOrMcpService` so the tool is actually reachable. Counters may be null — an idle card is runtime-suspended and unreadable, which is not 0%. WARP-1861. | false | false | orchestrator |
