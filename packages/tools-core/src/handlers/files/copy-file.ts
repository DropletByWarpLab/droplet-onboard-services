...
async function handler(args: Record<string, unknown>, ctx: ToolContext): Promise<ToolResult> {
  ...
  if (f.path === "/" || t.path === "/") return err("INVALID_PATH", "refusing to copy root");
  ...
  if (!res.ok) return err("COPY_FAILED", `nextcloud returned ${res.status}`);
...
const tool: Tool = {
  name: "copy_file",
  description:
    "Copy a file or directory to a new location (recursive for directories). Pass `from_path` and `to_path`. Fails if the destination exists; pass overwrite=true to replace.",
  inputSchema,
  requiresWrite: true,
