...
| `tls_cert_ips` | The self-signed TLS cert's IP SANs no longer cover the box's current interface IPs — the install-time cert froze the secrets-time harvest, so a box that changed networks warns forever (WARP-2132). Delegates the compare to `secrets.sh::_cert_covers_current_ips` (the SAME harvest the SAN builder uses), debounced to 2 consecutive runs seeing the SAME uncovered address set | Re-issues via `secrets.sh::_generate_tls_cert` — key REUSE (never `-newkey`), SAN superset (old IP SANs retained + current added), `.bootstrap` refreshed, gateway nginx hot-reloaded via `tls-reload.sh`. A public-CA leaf (ADR-023) is `ok` and never touched |
