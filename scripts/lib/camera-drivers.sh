...
persist_camera_modules() {
  ...
  # WARP-595: stage + rename. This write is guarded by the existence check
  # above, so a partial file from an interrupted run (some modules missing)
  # would otherwise be kept forever — every re-run says "already configured".
  # Staging to a .tmp and mv'ing means the guard only ever sees complete files.
  {
...
setup_udev_rules() {
  ...
  # WARP-595: stage + rename — same rationale as persist_camera_modules above
  # (existence-guarded write must never leave a keepable partial file). udev
  # only reads *.rules, so the transient .tmp is invisible to it.
  cat << 'RULES' | sudo tee "$rules.tmp" > /dev/null
