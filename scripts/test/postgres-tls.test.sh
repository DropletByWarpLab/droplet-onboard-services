...
start_db() {
  ...
  # defaults are computed at source time).
  log_info() { :; }; log_warn() { :; }; log_success() { :; }
  # shellcheck source=/dev/null
  REPO_ROOT="$sandbox" source "$REPO_ROOT_REAL/scripts/lib/internal-ca.sh"
  internal_ca_ensure
...
test_plaintext_tcp_rejected() {
  docker exec -e PGPASSWORD=pgtls-test-pw "$NAME" \
    psql "host=127.0.0.1 sslmode=disable user=droplet dbname=droplet" -c 'SELECT 1' 2>&1 \
    | grep -q 'no pg_hba.conf entry.*no encryption'
}
...
test_database_url_producers_pin_sslmode() {
  ...
  grep -q 'DATABASE_URL=postgresql://droplet:${POSTGRES_PASSWORD}@db:5432/droplet?sslmode=require' \
...
if _docker_up; then
  ...
  printf "\n  ${_YELLOW}Docker daemon unreachable — skipping the 3 dockerized TLS/SCRAM tests${_RESET}\n"
