...
|----------|---------|-------------|
| `JETSON_OLLAMA_URL` | `http://host.docker.internal:8002/proxy` | ollama-manager chat-proxy URL on the Jetson side. Provides tool-call observability + JSON repair + circuit breaker; exposes `/health.limits` so ai-gateway can size outbound concurrency. |
