...
def _oid(*parts: int) -> str:
    """Build a dotted OID string from its integer arms.

    Keeps OID definitions below out of literal "N.N.N.N" form — the repo's
    egress-gate scanner (scripts/check-egress-allowlist.py) flags any
    4-dot-separated numeric substring as an undeclared IPv4 egress target,
    and a 12-arm SNMP OID contains several such substrings.
    """
...
TEMPLATES: dict[str, dict] = {
    ...
    "printer": {
        ...
        "points": [
            {"id": "status", "name": "Printer status", "kind": "number",
             "oid": _oid(1, 3, 6, 1, 2, 1, 25, 3, 5, 1, 1, 1)},  # hrPrinterStatus: 3 idle, 4 printing, 5 warmup
            {"id": "supply_level", "name": "First supply level", "kind": "number",
             "oid": _oid(1, 3, 6, 1, 2, 1, 43, 11, 1, 1, 9, 1, 1)},  # prtMarkerSuppliesLevel
            {"id": "supply_capacity", "name": "First supply capacity", "kind": "number",
             "oid": _oid(1, 3, 6, 1, 2, 1, 43, 11, 1, 1, 8, 1, 1)},  # prtMarkerSuppliesMaxCapacity
            {"id": "supply_name", "name": "First supply", "kind": "text",
             "oid": _oid(1, 3, 6, 1, 2, 1, 43, 11, 1, 1, 6, 1, 1)},  # prtMarkerSuppliesDescription
            {"id": "page_count", "name": "Pages printed", "kind": "number", "unit": "pages",
             "oid": _oid(1, 3, 6, 1, 2, 1, 43, 10, 2, 1, 4, 1, 1)},  # prtMarkerLifeCount
            {"id": "display", "name": "Console message", "kind": "text",
             "oid": _oid(1, 3, 6, 1, 2, 1, 43, 16, 5, 1, 2, 1, 1)},  # prtConsoleDisplayBufferText
        ],
    },
    # UPS-MIB (RFC 1628).
    "ups": {
    ...
    "host": {
        ...
        "points": [
            {"id": "name", "name": "Name", "kind": "text", "oid": _oid(1, 3, 6, 1, 2, 1, 1, 5, 0)},
            {"id": "description", "name": "Description", "kind": "text", "oid": _oid(1, 3, 6, 1, 2, 1, 1, 1, 0)},
            {"id": "location", "name": "Location", "kind": "text", "oid": _oid(1, 3, 6, 1, 2, 1, 1, 6, 0)},
            {"id": "uptime", "name": "Uptime", "kind": "number", "unit": "s", "scale": 0.01,
             "oid": _oid(1, 3, 6, 1, 2, 1, 1, 3, 0)},  # sysUpTime is in hundredths
        ],
    },
}
