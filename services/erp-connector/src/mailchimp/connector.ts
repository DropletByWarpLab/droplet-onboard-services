...
const MAILCHIMP_DATASET_COLUMNS: Readonly<Record<string, readonly string[]>> = {
  ...
  // is what a watermark reads. A Mailchimp `contact` IS a list member, which is
  // why this dataset can honestly carry the column while `campaign` cannot.
  contact: ["contact_id", "email_address", "status", "last_changed", "opt_in_time", "updated_at"],
