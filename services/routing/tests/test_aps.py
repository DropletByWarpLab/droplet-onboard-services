...
class TestApWireless:
    """GET /aps/{mac}/wireless — WARP-1714.

    Verified against the live lab box 2026-08-04: the Pi edge router reports
    `radio0` with `interfaces: []` and its uci carries only a DISABLED
    `ssid='OpenWrt'` placeholder, so the household SSID exists ONLY on the AP.
    Reading the router alone leaves the dashboard's Wi-Fi card blank.
    """

    VALID_MAC = "B8:27:EB:DD:EE:FF"

    def _seed(self, ap_client: TestClient) -> None:
        resp = ap_client.post(
            "/aps/_test_seed",
            json={"mac": self.VALID_MAC, "last_ip": "192.168.9.179"},
            headers=AUTH,
        )
        assert resp.status_code == 200, resp.text

    def test_reports_the_wifi_the_ap_is_broadcasting(self, ap_client: TestClient) -> None:
        self._seed(ap_client)
        approve = ap_client.post(
            f"/aps/{self.VALID_MAC}/approve",
            json={"ssid": "Studio Fotonia", "encryption_key": "longenoughpw"},
            headers=AUTH,
        )
        assert approve.status_code == 200, approve.text

        resp = ap_client.get(f"/aps/{self.VALID_MAC}/wireless", headers=AUTH)
        assert resp.status_code == 200, resp.text
        body = resp.json()
        assert body["supported"] is True
        assert body["wireless"]["ssid"] == "Studio Fotonia"

    def test_an_unapproved_ap_reports_no_wireless_not_an_error(
        self, ap_client: TestClient
    ) -> None:
        self._seed(ap_client)
        resp = ap_client.get(f"/aps/{self.VALID_MAC}/wireless", headers=AUTH)
        assert resp.status_code == 200, resp.text
        assert resp.json()["wireless"] is None

    def test_404_for_a_mac_that_never_announced(self, ap_client: TestClient) -> None:
        resp = ap_client.get("/aps/B8:27:EB:11:22:33/wireless", headers=AUTH)
        assert resp.status_code == 404

    def test_503_when_disconnected(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(main, "router_instance", None)
        resp = TestClient(main.app).get(f"/aps/{self.VALID_MAC}/wireless", headers=AUTH)
        assert resp.status_code == 503
