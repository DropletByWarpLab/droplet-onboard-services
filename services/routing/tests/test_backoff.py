"""WARP-1510 — BackoffState semantics for the OpenWrt reconnect retry.

Mirrors services/email-indexer/tests/test_backoff.py's shape — same pure
capped-exponential-backoff state machine, reused here so the background
reconnect retry (reconnect.py) never spins: it grows the wait between
attempts and caps it instead of hammering a backend that stays down.
"""
