"""WARP-1023 — switch FIPS boot self-test wiring.
...
calls `_shared.fips_selftest.gated_assert_fips_at_boot("switch")` directly
at module-import time (WARP-229). So the env matrix is exercised against
the gated helper itself, and the required-path boot is exercised by
importing `main` in a subprocess (re-importing main in-process would build
a second module object and defeat monkeypatching in sibling tests — see
services/ai-gateway/tests/test_fips_boot.py's docstring).
...
# conftest.py already puts services/ on sys.path for `_shared.*` imports.
...
def test_skip_when_env_unset(capsys):
    assert fips_selftest.gated_assert_fips_at_boot("switch") is None
...
def test_cryptography_dependency_pinned():
    ...
    ), "cryptography pin missing from services/switch/requirements.txt"
