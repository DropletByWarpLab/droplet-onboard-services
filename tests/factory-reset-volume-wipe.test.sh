...
if grep -qE 'basename[[:space:]]+"?\$\(dirname[[:space:]]+"?\$COMPOSE_FILE' <<<"$CODE"; then
...
if grep -qE "grep[[:space:]].*'?\^name:.*\"?\\\$COMPOSE_FILE" <<<"$CODE"; then
...
if grep -qE 'docker[[:space:]]+volume[[:space:]]+ls' <<<"$CODE"; then
...
if grep -qE 'could not be removed' <<<"$CODE" \
   && grep -qE '^[[:space:]]*exit[[:space:]]+1' <<<"$CODE"; then
...
if grep -qE 'remove-orphans[[:space:]]+2>/dev/null' <<<"$CODE"; then
...
if grep -qE 'droplet-pi-platform' <<<"$CODE"; then
...
if grep -qE "'\^docker_'|\"\^docker_\"|-p[[:space:]]+docker[[:space:]]" <<<"$CODE"; then
...
# there IS one.
#
# WARP-2621: the rest of that note used to claim only pipelines with an
# EXTERNAL command before the grep were affected. That is not true. bash's
# printf builtin writes $CODE (28 KB) in several chunks, so it takes SIGPIPE
# too whenever the match lands early enough that `grep -q` exits mid-write —
# measured 1-3 per 300 for the pgdata / exit-1 / droplet-pi-platform
# assertions, and observed live as `verify does not reference pgdata` on a tree
# where it plainly does. Every `printf | grep -q` in this file is now a
# here-string, which is not a pipeline and cannot be affected either way.
...
if grep -qF 'com.docker.compose.project=' <<<"$CODE"; then
...
if grep -qF '_remaining_owned_containers' <<<"$CODE"; then
...
if grep -F 'com.docker.compose.project=' <<<"$CODE" | grep -F '$prefix' >/dev/null; then
