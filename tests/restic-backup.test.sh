...
STUB
  ...
  run_ensure() {
    ( set -euo pipefail
      ...
      # WARP-1405: the key-mismatch path now looks for previous keys in
      # <root>/.env.bak.*; point it at an empty root so a developer's real
      # checkout can never leak into these stubbed cases.
      export DROPLET_REPO_ROOT="$ENSURE_TMP"
      # shellcheck disable=SC1090
      source "$LIB_SCRIPT"
...
# =============================================================================
# Key lifecycle + explicit status (WARP-1405) — real restic, no Docker.
#
# The incident: DEVICE_SECRET_KEY rotated, the repository (keyed by a
# password derived from it) stopped opening, every nightly run failed in
# 0.5 s, and nothing told anyone for 16 days — twice. These cases pin:
#   1. a rotation that leaves the old key in .env.bak.* RE-KEYS the repository:
#      history kept, new key opens it, OLD key no longer does;
#   2. an orphaned repository (old key gone) is reported as the explicit
#      state key_mismatch — never ignored, never deleted, never re-initialized;
#   3. any other failure records state=failed with the phase that broke.
# docker is stubbed (droplet-backup.sh only checks it is on PATH before the
# key check); restic is real so the key operations are the real ones.
# =============================================================================
echo ""
echo "--- Key lifecycle + status (WARP-1405) ---"

if ! command -v restic >/dev/null 2>&1; then
  skip "restic not on PATH — key lifecycle checks"
elif ! command -v openssl >/dev/null 2>&1; then
  skip "openssl not on PATH — key lifecycle checks"
else
  KL="$(mktemp -d)"
  mkdir -p "$KL/root/docker" "$KL/bin" "$KL/src"
  echo 'name: droplet' > "$KL/root/docker/docker-compose.yml"
  printf '#!/usr/bin/env bash\nexit 1\n' > "$KL/bin/docker"   # every docker call fails
  chmod +x "$KL/bin/docker"
  echo "precious" > "$KL/src/file.txt"

  write_env() { printf '# generated by setup.sh\nDEVICE_SECRET_KEY=%s\n' "$1" > "$KL/root/.env"; }
  derive() { DEVICE_SECRET_KEY="$1" bash -c "source '$LIB_SCRIPT' && droplet_backup_derive_password"; }
  opens_with() { RESTIC_PASSWORD="$(derive "$1")" restic -r "$KL/repo" cat config >/dev/null 2>&1; }
  snap_count() { RESTIC_PASSWORD="$(derive "$1")" restic -r "$KL/repo" snapshots --json 2>/dev/null | grep -o '"short_id"' | wc -l | tr -d ' '; }
  kl_backup() {
    PATH="$KL/bin:$PATH" DROPLET_REPO_ROOT="$KL/root" \
    DROPLET_BACKUP_TARGET="$KL/repo" DROPLET_BACKUP_STATE_DIR="$KL/state" \
    DROPLET_BACKUP_RUNTIME_DIR="$KL/run" DROPLET_BACKUP_STATUS_DIR="$KL/status" \
      bash "$BACKUP_SCRIPT" "$@" >"$KL/out" 2>&1
  }
  status_is() { grep -q "\"state\": \"$1\"" "$KL/status/status.json" 2>/dev/null; }

  K1="$(openssl rand -base64 32)"; K2="$(openssl rand -base64 32)"; K3="$(openssl rand -base64 32)"

  # First contact: --check-key initializes + records `pending`, never `ok`.
  write_env "$K1"
  rc=0; kl_backup --check-key || rc=$?
  if [ "$rc" -eq 0 ] && status_is pending; then
    pass "first --check-key: repository initialized, state=pending (no backup claimed)"
  else
  ...
  # --- 1. Rotation with the old key in .env.bak.* → re-keyed ----------------
  cp "$KL/root/.env" "$KL/root/.env.bak.1700000000"
  write_env "$K2"
  rc=0; kl_backup --check-key || rc=$?
  if [ "$rc" -eq 0 ]; then pass "rotation: --check-key succeeds after DEVICE_SECRET_KEY rotation"
  else fail "rotation: --check-key rc=$rc"; sed 's/^/      /' "$KL/out"; fi
  opens_with "$K2" && pass "rotation: repository opens with the NEW key" || fail "rotation: new key does not open the repository"
  opens_with "$K1" && fail "rotation: OLD key still opens the repository (not removed)" \
    || pass "rotation: old key removed — it no longer opens the repository"
  [ "$(snap_count "$K2")" = "1" ] && pass "rotation: snapshot history kept (1 snapshot readable with the new key)" \
    || fail "rotation: snapshot history lost"
  grep -q '"lastRekeyAt": "20' "$KL/status/status.json" && pass "rotation: re-key recorded (lastRekeyAt)" \
    || fail "rotation: lastRekeyAt not recorded"
  # …and a backup under the new key works against the same repository.
  RESTIC_PASSWORD="$(derive "$K2")" restic -r "$KL/repo" backup -q "$KL/src" >/dev/null 2>&1 \
    && [ "$(snap_count "$K2")" = "2" ] && pass "rotation: next backup succeeds under the new key" \
    || fail "rotation: backup under the new key failed"

  # --- 2. Orphaned: old key gone (factory-reset shreds .env.bak.*) ----------
  rm -f "$KL/root/.env.bak."*
  write_env "$K3"
  rc=0; kl_backup --check-key || rc=$?
  if [ "$rc" -ne 0 ] && status_is key_mismatch; then
    pass "orphaned: exits non-zero and records state=key_mismatch"
  else
    fail "orphaned: rc=$rc status=$(cat "$KL/status/status.json" 2>/dev/null)"
  fi
  grep -q '"lastFailureAt": "20' "$KL/status/status.json" && pass "orphaned: lastFailureAt recorded" \
    || fail "orphaned: lastFailureAt missing"
  if [ -f "$KL/repo/config" ] && [ "$(snap_count "$K2")" = "2" ]; then
    pass "orphaned: repository left untouched (never deleted, moved or re-initialized)"
  else
    fail "orphaned: repository was modified"
  fi
  grep -q -- '--rekey-from' "$KL/out" && pass "orphaned: log names the --rekey-from recovery path" \
    || fail "orphaned: recovery path not printed"

  # --- 2b. Recovery path: operator supplies the old .env ------------------
  printf 'DEVICE_SECRET_KEY=%s\n' "$K2" > "$KL/old.env"
  rc=0; kl_backup --rekey-from "$KL/old.env" || rc=$?
  if [ "$rc" -eq 0 ] && opens_with "$K3" && status_is pending; then
    pass "recovery: --rekey-from re-keys an orphaned repository (state back to pending)"
  else
    fail "recovery: --rekey-from rc=$rc"; sed 's/^/      /' "$KL/out"
  fi

  # --- 3. Any other failure records `failed` + the phase ------------------
  rc=0; kl_backup || rc=$?   # full run: stub docker fails the pg_dump
  if [ "$rc" -ne 0 ] && status_is failed && grep -q 'dumping the databases' "$KL/status/status.json"; then
    pass "failure: a failed run records state=failed with the phase that broke"
  else
    fail "failure: rc=$rc status=$(cat "$KL/status/status.json" 2>/dev/null)"
  fi
  if grep -q '"lastSuccessAt": null' "$KL/status/status.json"; then
    pass "failure: no success is ever claimed (lastSuccessAt stays null)"
  else
    fail "failure: lastSuccessAt set without a completed backup"
  fi
  since_before="$(grep -o '"since": "[^"]*"' "$KL/status/status.json")"
  kl_backup || true
  [ "$(grep -o '"since": "[^"]*"' "$KL/status/status.json")" = "$since_before" ] \
    && pass "status: 'since' (the alert window anchor) survives later runs" \
    || fail "status: 'since' was overwritten"

  # setup.sh must run the key check in the same run that may have rotated.
  grep -q 'droplet-backup.sh --check-key' "$INSTALL_LIB" \
    && pass "install_restic_backup runs --check-key (re-key happens in the rotating setup run)" \
    || fail "install_restic_backup does not run --check-key"

  if [ "${KEEP_ARTIFACTS:-0}" = "1" ]; then info "kept key-lifecycle artifacts: $KL"; else rm -rf "$KL"; fi
fi
...
if ! command -v docker >/dev/null 2>&1 || ! have_docker; then
  ...
  run_backup() {
    ...
    DROPLET_BACKUP_STATUS_DIR="$STATE_DIR/status" \
      bash "$BACKUP_SCRIPT" "$@"
  }
  ...
  if ! wait_healthy db; then
    ...
    # WARP-1405: a real, completed backup is the ONLY thing that records ok.
    if grep -q '"state": "ok"' "$STATE_DIR/status/status.json" 2>/dev/null \
       && grep -q '"lastSuccessAt": "20' "$STATE_DIR/status/status.json"; then
      pass "successful backup recorded state=ok with lastSuccessAt"
    else
      fail "successful backup did not record ok: $(cat "$STATE_DIR/status/status.json" 2>/dev/null)"
    fi
